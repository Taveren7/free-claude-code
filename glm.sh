#!/bin/bash
# GLM 5.2 via Free Claude Code — single command launcher (skip permissions)
...
# Run Claude Code from wherever the user called this, with GLM-5.2 custom token suffix
cd "$ORIGINAL_DIR"
ANTHROPIC_AUTH_TOKEN="freecc:open_router/z-ai/glm-5.2" ANTHROPIC_BASE_URL="http://localhost:$PORT" claude --dangerously-skip-permissions "$@"
